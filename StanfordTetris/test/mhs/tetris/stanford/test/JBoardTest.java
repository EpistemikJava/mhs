 ...
 * -----------------------------------------------
 * $File: //depot/Eclipse/Java/workspace/StanfordTetris/test/mhs/tetris/stanford/test/JBoardTest.java $
 * $Revision: #1 $ 
 * $Change: 160 $ 
...
class JBoardTest extends JComponent
{
  ...
  /**
   * Whenever the user clicks a button, this runs to make the change. The strategy is:
   * compute what the new piece position should be, undo to remove the old piece position,
    ...
    for( y = 0; y < bMaxHeight; y++ )
    {
      ...
      for( x = 0; x < bWidth; x++ )
      {
        if( board.getGrid( x, y ) )
        {
          ...
          g.fillRect( xPixelDist( x, dx ) + 1, pixelHeight - yPixelDist( y, dy ) + 1,
              (int)dx - 2, (int)dy - 2 );
