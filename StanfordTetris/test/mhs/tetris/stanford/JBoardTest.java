 ...
 * -----------------------------------------------
 * $File: //depot/Eclipse/Java/workspace/StanfordTetris/test/mhs/tetris/stanford/JBoardTest.java $
 * $Revision: #2 $ 
 * $Change: 165 $ 
 * $DateTime: 2012/01/02 13:10:31 $
 * -----------------------------------------------
 * 
 * mhs.tetris.stanford.JBoardTest.java 
 ...
 * tetris classes. Its interface only make sense if you think of it as a direct connection
 * to the Board and Piece interfaces. JBoardTest is provided in finished form so the
 * students can use it to test their Piece and Board code.
 * <p>
 * Use the buttons on screen or the keys 4, 5, 6, 7, 0 for control. Add a piece, move it
 * around. JBoardTest uses place() and undo() to move the piece around the board. Use the
 * app to set up and test out specific cases for your tetris board and piece.
 * <p>
 * <b>Transparency</b> -- Board has all sorts of hidden state -- heights, widths etc.. The
 * hidden state is what makes Board hard to debug. The unit test tries to make the state
...
class JBoardTest extends JComponent
{
   ...
   *   
   * @param verb - action to perform
   */
  void action( int verb )
  {
    if( verb == RESET )
    {
      reset();
      ...
      switch( verb )
      {
        ...
        case DROP:
          ...
          if( piece != null )
          {
            ...
            // trick: avoid the case where the piece appears to move upwards
            if( $newY > pieceY )
              $newY = pieceY;
          }
          break;
  ...
  /**
   * Pixel helpers -- these centralize how we map from block coords (x,y) to pixel co-ords
   * on screen. By centralizing here, at least it's all consistent. These return the
   * distance in pixels from the left or botton edge to the upper left corner of the rect
   * for each tetris block.
   */
  private int xPixelDist( int x, float dx ) { return ( MARGIN + Math.round( x * dx ) ); }
  
  private int yPixelDist( int y, float dy ) { return ( Math.round( ( y + 1 ) * dy ) ); }
  ...
  public void paintComponent( Graphics page )
  {
    ...
    page.drawRect( MARGIN, 0, $pixelWidth - 1, $pixelHeight - 1 );
    ...
    for( y = 0; y < $bMaxHeight; y++ )
    {
      ...
      page.drawString( Integer.toString( board.getRowWidth( y ) ), 4, $pixelHeight - yPixelDist( y - 1, $dy ) - 4 );
      ...
      for( x = 0; x < $bWidth; x++ )
      {
        if( board.getGrid( x, y ) )
        {
          ...
          page.fillRect( xPixelDist( x, $dx ) + 1, $pixelHeight - yPixelDist( y, $dy ) + 1, (int)$dx - 2, (int)$dy - 2 );
    ...
    for( x = 0; x < $bWidth; x++ )
    {
      page.drawString( Integer.toString( board.getColumnHeight( x ) ), xPixelDist( x, $dx ) + 6, $pixelHeight + MARGIN - 4 );
    }
  ...
  public java.awt.Container createControlPanel()
  {
    ...
    // Register the 4 keystroke listeners to call action() with the appropriate constant.
  ...
  /** Create a frame for the board tester and its controls  
